...
/// 批量将产品名称转换为条形码
Future<List<Map<String, dynamic>>> batchConvertNamesToBarcodes(
  List<Map<String, dynamic>> purchasedItems,
) async {
  try {
    return await _apiService.batchConvertNamesToBarodes(purchasedItems);
  } catch (e) {
    print("Error converting names to barcodes: $e");
    return [];
  }
}
    ...
    if (userId > 0) {
      try {
        ...
        if (recommendationResult != null && 
          ...
          // Extract LLM insights from response.data.llmInsights 或 llmAnalysis (兼容不同字段名)
          ...
          } else if (responseData['llmAnalysis'] != null) {
            llmInsights = responseData['llmAnalysis'] as Map<String, dynamic>?;
            print('📊 API: Found llmAnalysis field in data (using as llmInsights)');
          } else {
            print('⚠️ API: No llmInsights or llmAnalysis field found in response.data');
          }
          ...
          if (llmInsights != null && llmInsights.isNotEmpty) {
            summary = llmInsights['summary']?.toString() ?? '';
            detailedAnalysis = llmInsights['detailedAnalysis']?.toString() ?? '';
            actionSuggestions = (llmInsights['actionSuggestions'] as List?)
                ?.map((s) => s.toString())
                .toList() ?? <String>[];
            
            print('🎯 API: LLM Data Raw - Summary: "${summary}", Analysis: "${detailedAnalysis}", Actions: ${actionSuggestions.length}');
          } else {
            print('⚠️ API: No LLM insights available, using fallback data');
          }
      
          // 始终创建增强产品，包含推荐列表（无论是否有LLM数据）
          final enhancedProduct = ProductAnalysis(
            ...
            summary: summary.isNotEmpty ? summary : 'Product information loaded successfully.Allergens detected: ${basicProduct.detectedAllergens.join(', ')}.',
            detailedAnalysis: detailedAnalysis.isNotEmpty ? detailedAnalysis : 'Main ingredients include: ${basicProduct.ingredients.join(', ')}',
            actionSuggestions: actionSuggestions.isNotEmpty ? actionSuggestions : ['If you are allergic to ${basicProduct.detectedAllergens.join(', ')}, please avoid consuming this product', 'Please check the complete ingredient list', 'Consume in moderation as part of a balanced diet'],
            llmAnalysis: llmInsights,
            recommendations: recommendationsList, // 添加推荐产品列表
            barcode: barcode, // 确保包含条码信息
          );
          
          print('✅ API: Enhanced product created with ${recommendationsList.length} recommendations and ${llmInsights != null ? 'LLM' : 'fallback'} data');
          
          // 保存扫描历史记录到用户历史表
          try {
            // 准备LLM分析数据 (转换为JSON字符串)
            final llmAnalysisJson = jsonEncode(llmInsights ?? {});
            // 准备推荐产品数据 (转换为JSON字符串)
            final recommendedProductsJson = jsonEncode(
            ...
            await _apiService.saveBarcodeHistory(
              userId: userId,
              barcode: barcode,
              scanTime: DateTime.now().toIso8601String(),
              recommendationId: responseData['recommendationId']?.toString(),
              llmAnalysis: llmAnalysisJson,
              recommendedProducts: recommendedProductsJson,
            );
            
            print('✅ API: Scan history saved successfully');
          } catch (historyError) {
...
Future<Map<String, dynamic>> uploadReceiptImage(XFile imageFile, int userId) async {
  try {
    ...
    if (result != null) {
      // 保存扫描历史 - 已废弃：历史记录现在通过专用的 receipt-history 接口自动保存
...
Future<bool> checkLoyaltyUserExists(String userId) async {
  try {
    return await _apiService.checkUserExists(userId);
  } catch (e) {
    print("Error checking loyalty user exists: $e");
    return false;
  }
}

/// Get current points for a user
Future<int> getLoyaltyUserPoints(String userId) async {
  try {
    return await _apiService.getUserPoints(userId);
  } catch (e) {
    print("Error getting loyalty user points: $e");
    return 0;
  }
}

/// Award points to a user (owner only)
Future<Map<String, dynamic>?> awardLoyaltyPoints(String userId, int amount) async {
  try {
    return await _apiService.awardPoints(userId, amount);
  } catch (e) {
...
Future<Map<String, dynamic>?> redeemLoyaltyPoints(String userId) async {
  try {
    return await _apiService.redeemPoints(userId);
  } catch (e) {
...
Future<Map<String, dynamic>?> getLoyaltyContractInfo() async {
  try {
    return await _apiService.getContractInfo();
  } catch (e) {
    print("Error getting loyalty contract info: $e");
    return null;
  }
}