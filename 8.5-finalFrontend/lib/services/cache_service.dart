      ...
      for (final key in keys) {
        ...
        if (key is String && 
          ...
          try {
            ...
            // 特别处理类型错误
            if (keyError.toString().contains('type') && keyError.toString().contains('String')) {
              print('🔧 Detected type conversion error for key: $key');
              try {
                // 尝试获取原始值并重新存储
                final rawValue = _prefs!.get(key);
                if (rawValue != null) {
                  await _prefs!.setString(key, rawValue.toString());
                  print('🔧 Fixed type conversion for key: $key');
                } else {
                  await _prefs!.remove(key);
                  print('🗑️ Removed problematic key: $key');
                }
                removedCount++;
              } catch (fixError) {
                print('❌ Failed to fix type conversion for key $key: $fixError');
                // 最后尝试直接删除
                try {
                  await _prefs!.remove(key);
                  removedCount++;
    ...
    try {
      ...
      try {
        ...
        if (userId != null && userId > 0) {
          // 将整数类型的 user_id 转换为字符串类型存储
          await _prefs!.setString('user_id', userId.toString());
          print('🔧 Fixed user_id type conversion: $userId -> "${userId.toString()}"');
          removedCount++;
        }
      } catch (e) {
        print('❌ Error processing user_id type conversion: $e');
      }
