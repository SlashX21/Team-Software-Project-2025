...
class _HistoryDetailPageState extends State<HistoryDetailPage> {
  ...
  List<String>? _parseIngredientsString(String? ingredientsText) {
    ...
    String text = ingredientsText.trim();
    
    // 先尝试智能分割 - 考虑括号内的逗号不应该分割
    List<String> ingredients = _smartSplitIngredients(text);
    ...
    return cleanedIngredients.isEmpty ? null : cleanedIngredients;
  }
  
  /// 智能分割ingredients - 考虑括号内容
  List<String> _smartSplitIngredients(String text) {
    ...
    for (int i = 0; i < text.length; i++) {
      ...
      if (c == '(') {
        ...
        parenCount--;
        // Fix negative parentheses count (unmatched closing parenthesis)
        if (parenCount < 0) {
          parenCount = 0;
        }
      } else if (c == ',' && parenCount == 0) {
        // 只有在括号外的逗号才分割
        String ingredient = text.substring(start, i).trim();
    ...
    // 添加最后一个成分
    String lastIngredient = text.substring(start).trim();
    if (lastIngredient.isNotEmpty) {
      result.add(lastIngredient);
    }
    
    // 修复未闭合的括号
    result = _fixUnmatchedParentheses(result);
    
    // 如果智能分割没有结果，尝试简单分割
    if (result.isEmpty || (result.length == 1 && result[0] == text)) {
      return text.split(RegExp(r'[,;]')).map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
    }
  ...
  /// 修复未闭合的括号
  List<String> _fixUnmatchedParentheses(List<String> ingredients) {
    List<String> fixed = [];
