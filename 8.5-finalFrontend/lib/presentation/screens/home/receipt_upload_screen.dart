...
class _ReceiptUploadScreenState extends State<ReceiptUploadScreen> {
  ...
  Future<void> _uploadReceipt() async {
    ...
    try {
      ...
      print('📦 Prepared items with names: $purchasedItemsWithNames');
      ...
      try {
        ...
        final recommendationResult = await getReceiptAnalysis(
          userId: _userId!,
          purchasedItems: purchasedItemsWithNames, // 直接使用产品名称格式，Java后端期望的格式
        );
  ...
  Future<void> _retryRecommendationAnalysis() async {
    ...
    try {
      ...
      print('📦 Retry: Prepared items with names: $purchasedItemsWithNames');
  ...
  Widget _buildLLMInsightsCard() {
    ...
    print('🔍 LLM Insights Debug: $llmInsights');
    print('🔍 Full Recommendation Data: $_recommendationData');
  ...
  Widget _buildKeyFindingsSection(dynamic keyFindings) {
    ...
    if (keyFindings is List) {
      ...
      keyFindingsText = findings.map((f) => '• $f').join('\n');
    } else {
  ...
  Widget _buildImprovementSuggestionsSection(dynamic suggestions) {
    ...
    if (suggestions is List) {
      ...
      suggestionsText = suggestionsList.map((s) => '• $s').join('\n');
    } else {
