...
class _ComprehensiveNutritionGoalsPageState extends State<ComprehensiveNutritionGoalsPage>
  ...
  void _calculateRecommendedGoals() {
    ...
    setState(() {
      ...
      // Micronutrients
      _dailyFiber = (_age ?? 30) < 50 ? 25 : 21; // Adjust based on age
  ...
  double _calculateBMR() {
    ...
    final age = _age ?? 30; // Use default age if null
    if (_gender == 'male') {
  ...
  Future<void> _saveGoals() async {
    ...
    try {
      ...
      if (_userProfile != null) {
        if (_userProfile!['age'] != _age) {
          if (_age != null) {
            personalInfoUpdates['age'] = _age;
          }
        }
      ...
      // 保存糖分目标
      await setSugarGoal(userId, _dailySugar * 1000, 'CUSTOM'); // 转换为mg，添加 goalLevel 参数
