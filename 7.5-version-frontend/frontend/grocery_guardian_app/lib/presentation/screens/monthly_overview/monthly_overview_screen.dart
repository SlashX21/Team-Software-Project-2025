  ...
  void _loadMonthlyData() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        ...
        MockMonthlyOverviewService.getMockpurchaseSummary(
          userId: userId,
          year: _pageState.selectedYear,
          month: _pageState.selectedMonth,
        ),
        MockMonthlyOverviewService.getMockNutritionInsights(
          userId: userId,
          year: _pageState.selectedYear,
          month: _pageState.selectedMonth,
        ),
        MockMonthlyOverviewService.getMockHealthInsights(
          userId: userId,
          year: _pageState.selectedYear,
          month: _pageState.selectedMonth,
        ),
      ...
      setState(() {
        _pageState = _pageState.copyWith(
          isLoading: false,
          overview: futures[0] as MonthlyOverview?,
          purchaseSummary: futures[1] as MonthlypurchaseSummary?,
          nutritionInsights: futures[2] as MonthlyNutritionInsights?,
          healthInsights: futures[3] as List<HealthInsight>? ?? [],
        );
      });
    } catch (e) {
      setState(() {
          ...
          Expanded(
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                value: '${monthNames[_pageState.selectedMonth]} ${_pageState.selectedYear}',
                onChanged: (value) {
                ...
                items: [
                  DropdownMenuItem(
                    value: '${monthNames[_pageState.selectedMonth]} ${_pageState.selectedYear}',
                    child: Text('${monthNames[_pageState.selectedMonth]} ${_pageState.selectedYear}'),
                  ),
                ],
              ),
            ),
