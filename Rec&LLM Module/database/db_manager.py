...
class SQLiteAdapter(DatabaseAdapter):
    ...
    def get_product_by_barcode(self, barcode: str) -> Optional[Dict]:
        ...
        try:
            ...
            query = """
            SELECT barcode, name, brand, ingredients, allergens,
    ...
    def get_user_profile(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            cursor = self.connection.cursor()
            query = """
            SELECT user_id, username, email, age, gender, height_cm, weight_kg,
    ...
    def get_purchase_history(self, user_id: int, days: int = 90) -> List[Dict]:
        ...
        try:
            ...
            SELECT pr.purchase_id, pr.user_id, pr.receipt_date, pr.store_name,
                   pr.total_amount, pi.item_id, pi.barcode, pi.item_name_ocr,
                   pi.quantity, pi.unit_price, pi.total_price,
                   p.name, p.brand, p.category, p.energy_kcal_100g, p.proteins_100g
    ...
    def get_user_purchase_matrix(self, user_id: int) -> Dict:
        ...
        try:
            ...
            # 获取用户购买的商品及权重
            query = """
            SELECT pi.barcode, p.name, p.category,
            ...
            LEFT JOIN PRODUCT p ON pi.barcode = p.barcode
            WHERE pr.user_id = ?
            GROUP BY pi.barcode, p.name, p.category
            ...
            # 计算相似用户（基于共同购买商品）
            similar_users_query = """
            SELECT pr2.user_id, COUNT(DISTINCT pi2.barcode) as common_products
            FROM PURCHASE_RECORD pr1
            JOIN PURCHASE_ITEM pi1 ON pr1.purchase_id = pi1.purchase_id
            JOIN PURCHASE_ITEM pi2 ON pi1.barcode = pi2.barcode
    ...
    def check_product_allergens(self, barcode: str, user_allergens: List[int]) -> Dict:
        ...
        try:
            ...
            WHERE pa.barcode = ? AND pa.allergen_id IN ({placeholders})
    ...
    def log_recommendation(self, log_data: Dict) -> bool:
        ...
        try:
            ...
            INSERT INTO RECOMMENDATION_LOG (
                user_id, request_barcode, request_type, recommended_products,
    ...
    def get_recommendation_stats(self, user_id: int) -> Dict:
        ...
        try:
            ...
            SELECT 
                ...
                COUNT(DISTINCT request_barcode) as unique_products_scanned,
