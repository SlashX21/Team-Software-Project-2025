...
class ProgressiveLoader {
  ...
  Future<void> _loadRecommendationsAsync(
    ...
    try {
      ...
      // 增加超时时间到20秒，因为LLM推荐系统需要更多处理时间（实际处理时间约13-15秒）
      final completeProduct = await fetchProductByBarcode(barcode, userId)
        ...
        throw TimeoutException('LLM recommendations timed out', Duration(seconds: 20));
      });
  ...
  ProductAnalysis _enhanceProductWithFallback(ProductAnalysis? llmProduct, ProductAnalysis basicProduct) {
    print('🔍 Progressive Loader: Checking LLM data availability');
    print('🔍 Progressive Loader: llmProduct is null: ${llmProduct == null}');
    
    if (llmProduct != null) {
      print('🔍 Progressive Loader: summary.isNotEmpty: ${llmProduct.summary.isNotEmpty} (length: ${llmProduct.summary.length})');
      print('🔍 Progressive Loader: detailedAnalysis.isNotEmpty: ${llmProduct.detailedAnalysis.isNotEmpty} (length: ${llmProduct.detailedAnalysis.length})');
      print('🔍 Progressive Loader: actionSuggestions.isNotEmpty: ${llmProduct.actionSuggestions.isNotEmpty} (length: ${llmProduct.actionSuggestions.length})');
      print('🔍 Progressive Loader: recommendations.isNotEmpty: ${llmProduct.recommendations.isNotEmpty} (length: ${llmProduct.recommendations.length})');
    }
    ...
    if (llmProduct != null && 
      ...
      // LLM data available (包括推荐产品), use LLM data
      print('✅ Progressive Loader: Using LLM data with ${llmProduct.recommendations.length} recommendations');
      return llmProduct;
    }
    
    // LLM data not available, use local enhancement
    print('⚠️ Progressive Loader: LLM data not available, using fallback');
