...
class ApiService {
  ...
  Future<Map<String, dynamic>?> getReceiptAnalysis({
    ...
    try {
      ...
      print('📍 RECOMMENDATION: Target URL: $recommendationBaseUrl/recommendations/receipt');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          print('⚠️ RECOMMENDATION: Analysis failed - ${data['message']}');
          return data; // Return the error data for frontend to handle
        }
      } else {
  ...
  Future<Map<String, dynamic>?> scanReceipt(XFile imageFile) async {
    try {
      print('📤 OCR: Starting image upload...');
      print('📄 OCR: Image file name: ${imageFile.name}');
      print('📍 OCR: Target URL: $baseUrl/ocr/scan');
      ...
      print('🚀 OCR: Sending request to backend...');
      final response = await request.send();
      final responseBody = await response.stream.bytesToString();
      
      print('📡 OCR: Response status code: ${response.statusCode}');
      print('📄 OCR: Response body: $responseBody');

      if (response.statusCode == 200) {
        final data = jsonDecode(responseBody);
        print('✅ OCR: Successfully parsed response data');
        return data['data'];
      } else {
        print('❌ OCR: Non-200 status code: ${response.statusCode}');
        print('❌ OCR: Error response body: $responseBody');
      }
      return null;
    } catch (e) {
      print('❌ OCR: Exception occurred: $e');
      print('❌ OCR: Exception type: ${e.runtimeType}');
      return null;
    }
  }
