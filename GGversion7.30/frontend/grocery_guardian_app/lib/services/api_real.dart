...
class RealApiService {
  ...
  static Future<Map<String, dynamic>> uploadReceiptImage(XFile imageFile, int userId) async {
    try {
      ...
      // Check OCR service availability first
      try {
        final healthCheck = await http.get(
          Uri.parse('${ApiConfig.ocrBaseUrl}/health'),
          headers: {'Content-Type': 'application/json'},
        ).timeout(Duration(seconds: 3));
        
        if (healthCheck.statusCode != 200) {
          print("⚠️ OCR service health check failed: ${healthCheck.statusCode}");
        }
      } catch (e) {
        print("❌ OCR service is not available at ${ApiConfig.ocrBaseUrl}");
        throw Exception('OCR service is currently unavailable. Please try again later.');
      }
      
      // Step 1: Upload to OCR service with optimized settings
    ...
    final result = {
      ...
      'llmInsights': {
        'summary': purchasedItems.isNotEmpty 
          ? 'Successfully extracted ${purchasedItems.length} items from your receipt'
