...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  /// 🔧 Async allergen data loading - doesn't block basic profile display
  Future<void> _loadAllergenDataAsync(int userId) async {
    try {
      print('🔄 Loading allergen data asynchronously...');
      
      final results = await Future.wait([
        ApiService.getUserAllergens(userId: userId).timeout(Duration(seconds: 5)),
        _getAllAllergens().timeout(Duration(seconds: 5)),
      ]);
      
      final userAllergens = results[0] as List<Map<String, dynamic>>?;
      final allAllergens = results[1] as List<Map<String, dynamic>>?;
      ...
      if (mounted) {
        ...
        print('✅ Allergen data loaded asynchronously');
      }
    } catch (e) {
      print('⚠️ Allergen data loading failed (non-critical): $e');
      
      if (mounted) {
        setState(() {
          _userAllergens = [];
          _allAllergens = _getFallbackAllergens(); // Use fallback data
        });
        
        // Show non-intrusive notification
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('过敏原信息暂时无法获取，已显示基本过敏原列表'),
  ...
  Future<void> _loadUserProfile() async {
    ...
    try {
      ...
      // 🔧 Decouple profile loading: prioritize basic info, load allergens separately
      monitor.startTimer('profile_api_calls');
      
      // Step 1: Load basic user profile first (critical)
      final userData = await ApiService.getUserProfile(userId: userId);
      
      if (userData != null) {
        // Update UI with basic profile data immediately
        setState(() {
          _userProfile = userData;
          _passwordHash = userData['passwordHash'];
          _isLoading = false; // Basic profile loaded successfully
        });
        
        print('✅ Basic profile loaded successfully');
        
        // Step 2: Load allergen data asynchronously (non-critical)
        _loadAllergenDataAsync(userId);
        
        final totalTime = monitor.endTimer('profile_load_total');
        print('✅ Profile loading initiated in ${totalTime.inMilliseconds}ms');
