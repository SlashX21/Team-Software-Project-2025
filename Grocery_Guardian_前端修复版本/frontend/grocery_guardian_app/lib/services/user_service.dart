...
class UserService {
  ...
  Future<bool> saveUserInfo(Map<String, dynamic> userData) async {
    try {
      ...
      // 🔧 先清理可能存在的无效cache keys
      await _cleanupInvalidCacheKeys(prefs);
      ...
      // 进行更安全的数据类型转换，添加详细日志
      int? userIdToSave;
      print('🔍 Processing userId from backend data:');
      print('   Raw value: $rawUserId (type: ${rawUserId.runtimeType})');
      
      if (rawUserId is int) {
        userIdToSave = rawUserId;
        print('   ✅ Direct int assignment: $userIdToSave');
      } else if (rawUserId is String) {
        userIdToSave = int.tryParse(rawUserId);
        print('   🔄 String->int conversion: "$rawUserId" -> $userIdToSave');
      } else if (rawUserId is double) {
        userIdToSave = rawUserId.toInt();
        print('   🔄 Double->int conversion: $rawUserId -> $userIdToSave');
      } else {
        print('   ❌ Unexpected type: ${rawUserId.runtimeType}');
      }

      // 如果最终无法解析出有效的userId，则保存失败
      if (userIdToSave == null || userIdToSave <= 0) {
        print('❌ Failed to parse valid user ID from backend data.');
  ...
  /// 🔧 清理无效的cache keys (修复"type 'int' is not a subtype of type 'String?'"错误)
  Future<void> _cleanupInvalidCacheKeys(SharedPreferences prefs) async {
    try {
      final keys = prefs.getKeys().toList();
      int removedCount = 0;
      
      print('🔍 Checking ${keys.length} cache keys for type issues...');
      
      for (final key in keys) {
        if (key is! String) {
          print('🚫 Found invalid cache key type: $key (type: ${key.runtimeType})');
          try {
            await prefs.remove(key);
            removedCount++;
            print('✅ Removed invalid key: $key');
          } catch (e) {
      ...
      print('❌ Error during cache key cleanup: $e');
    }
  }
}