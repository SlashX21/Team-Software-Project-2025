...
// 🔧 Granular exception types for better error handling
      ...
      try {
        ...
        if (response.statusCode == 200) {
          try {
            ...
            if (userData != null) {
              // 🔧 Critical Fix: Ensure userId is converted to String at API parsing layer
              if (userData.containsKey('userId')) {
                userData['userId'] = userData['userId'].toString();
              } else if (userData.containsKey('user_id')) {
                userData['user_id'] = userData['user_id'].toString();
              } else if (userData.containsKey('id')) {
                userData['id'] = userData['id'].toString();
              }
              
              print("✅ Login successful via ${attempt['url']}");
              print("🔧 UserId normalized to String: ${userData['userId'] ?? userData['user_id'] ?? userData['id']}");
              return userData;
            } else {
          ...
          print("❌ Conflict - Duplicate user or registration conflict");
          throw ConflictException('用户名已存在或注册冲突');
        } else if (response.statusCode == 500) {
          print("⚠️ Internal server error (${response.statusCode}): ${response.body}");
          ...
          try {
            ...
            if (message.contains('Query did not return a unique result')) {
              ...
              throw InternalServerException('数据库完整性问题，请联系系统管理员');
              ...
              throw InternalServerException('服务器内部错误，请稍后重试');
            }
          } catch (parseError) {
            print("⚠️ Could not parse 500 error message: $parseError");
            // 🔧 Fallback: Check raw response body for user-related keywords
            final rawBody = response.body.toLowerCase();
            if (rawBody.contains('user') && (rawBody.contains('not found') || rawBody.contains('not exist') || rawBody.contains('不存在'))) {
              print("👤 User not found detected in raw response body");
              throw UserNotFoundException('用户不存在');
            }
            throw InternalServerException('服务器内部错误，请稍后重试');
          }
        ...
        // 🔧 Enhanced exception handling - distinguish business logic from network errors
        if (e is AuthenticationException || 
            e is UserNotFoundException || 
            e is ConflictException ||
            e is InternalServerException ||
            e is ServerException) {
          // Business logic errors - propagate immediately, don't try other endpoints
  ...
  static Future<Map<String, dynamic>?> getUserProfile({required int userId}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // 缓存用户数据
        if (userData != null) {
          // 🔧 Critical Fix: Ensure userId is String in profile data
          if (userData.containsKey('userId')) {
            userData['userId'] = userData['userId'].toString();
          } else if (userData.containsKey('user_id')) {
            userData['user_id'] = userData['user_id'].toString();
          ...
          print("🔧 Profile userId normalized to String: ${userData['userId'] ?? userData['user_id'] ?? userData['id']}");
        }
