...
from config.settings import get_recommendation_config, get_database_config
...
class RecommendationEngine:
    ...
    def __init__(self, db_manager: Optional[DatabaseManager] = None,
        ...
        logger.info(f"DB_TYPE: {os.getenv('DB_TYPE', 'Not set')}")
        logger.info(f"JAVA_DB_CONNECTION_STRING: {os.getenv('JAVA_DB_CONNECTION_STRING', 'Not set')}")
        ...
        logger.info(f"Database connection: {db_config.connection_string}")
        logger.info(f"Database adapter type: {type(self.db.adapter).__name__}")
    ...
    async def recommend_alternatives(self, request: BarcodeRecommendationRequest) -> RecommendationResponse:
        ...
        try:
            ...
            if not self.db.adapter.connection:
                logger.info("Database connection not found, connecting...")
                self.db.connect()
            
            # 1. Get original product information
            logger.info(f"Step 1: Getting original product for barcode: {request.product_barcode}")
            original_product = self.db.get_product_by_barcode(request.product_barcode)
            if not original_product:
                logger.error(f"Product not found for barcode: {request.product_barcode}")
            ...
            logger.info(f"Step 2: Getting user profile for user ID: {request.user_id}")
            user_profile = self.db.get_user_profile(request.user_id)
            if not user_profile:
                logger.error(f"User not found for user ID: {request.user_id}")
                return RecommendationResponse(
                    recommendation_id=recommendation_id,
                    scan_type="barcode_scan",
                    user_profile_summary={},
            ...
            logger.info(f"User profile retrieved successfully: {user_profile.get('userName', 'Unknown')}")
            logger.info(f"User nutrition goal: {user_profile.get('nutritionGoal', 'Unknown')}")
            
            # Get user allergens
            logger.info(f"Step 2.1: Getting user allergens for user ID: {request.user_id}")
            user_allergens = self.db.get_user_allergens(request.user_id)
            logger.info(f"User allergens: {', '.join([a['name'] for a in user_allergens]) if user_allergens else 'None'}")
            
            # 3. Get candidate products
            logger.info(f"Step 3: Getting candidate products for category: {original_product.get('category', 'Unknown')}")
            ...
            if len(candidates) == 0:
                logger.warning("No candidate products found, returning empty recommendations")
                return RecommendationResponse(
                    recommendation_id=recommendation_id,
                    scan_type="barcode_scan",
                    ...
                    success=True,
                    message="No candidate products found in same category"
                )
            
            # 4. Apply hard filters
            logger.info(f"Step 4: Applying hard filters to {len(candidates)} candidates")
            ...
            logger.info(f"Step 4.1: Applying smart subcategory filtering")
            ...
            logger.info(f"Step 5: Optimizing nutrition for {len(candidates_to_optimize)} candidates")
            ...
            logger.info(f"Step 6: Generating LLM analysis")
