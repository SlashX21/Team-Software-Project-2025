...
class MySQLAdapter(DatabaseAdapter):
    ...
    def search_products_by_nutrition(self, filters: Dict) -> List[Dict]:
        ...
        try:
            ...
            SELECT bar_code, product_name, brand, category,
                   energy_kcal_100g, proteins_100g, fat_100g, sugars_100g,
                   carbohydrates_100g, serving_size
            FROM product 
            WHERE 1=1
            """
            ...
            if filters.get("max_calories"):
                base_query += " AND energy_kcal_100g <= :max_calories"
                params["max_calories"] = filters["max_calories"]
            
            if filters.get("min_protein"):
                base_query += " AND proteins_100g >= :min_protein"
                params["min_protein"] = filters["min_protein"]
            
            if filters.get("max_fat"):
                base_query += " AND fat_100g <= :max_fat"
                params["max_fat"] = filters["max_fat"]
            
            if filters.get("max_sugar"):
                base_query += " AND sugars_100g <= :max_sugar"
                params["max_sugar"] = filters["max_sugar"]
            ...
            # 添加排序
            base_query += " ORDER BY energy_kcal_100g ASC"
        ...
        try:
            ...
            if row:
                ...
                if user_data.get("age") is None:
                    user_data["age"] = 30
                if user_data.get("height_cm") is None:
                    user_data["height_cm"] = 170
                if user_data.get("weight_kg") is None:
                    user_data["weight_kg"] = 70.0
                if user_data.get("activity_level") is None:
                    user_data["activity_level"] = "moderate"
                if user_data.get("nutrition_goal") is None:
                    user_data["nutrition_goal"] = "maintain"
                if user_data.get("daily_calories_target") is None:
                    user_data["daily_calories_target"] = 2000.0
                if user_data.get("daily_protein_target") is None:
                    user_data["daily_protein_target"] = 150.0
                if user_data.get("daily_carb_target") is None:
                    user_data["daily_carb_target"] = 250.0
                if user_data.get("daily_fat_target") is None:
                    user_data["daily_fat_target"] = 65.0
                
                logger.info(f"Found user profile for user ID {user_id}: {user_data['user_name']}")
                return user_data
            else:
                logger.error(f"User not found for user ID: {user_id}")
                return None
                
        except Exception as e:
            logger.error(f"MySQL查询用户档案失败 {user_id}: {e}")
            return None
    ...
    def get_user_allergens(self, user_id: int) -> List[Dict]:
        ...
        try:
            from sqlalchemy import text
            query = text("""
            SELECT ua.user_allergen_id, ua.user_id, ua.allergen_id, ua.severity_level,
                   ua.confirmed, ua.notes, a.name as allergen_name, a.category as allergen_category
            FROM user_allergen ua
            JOIN allergen a ON ua.allergen_id = a.allergen_id
            ...
            allergens = []
            for row in rows:
                allergen_data = dict(row._mapping)
                allergens.append(allergen_data)
            
            logger.info(f"Found {len(allergens)} allergens for user ID {user_id}")
            return allergens
    ...
    def get_purchase_history(self, user_id: int, days: int = 90) -> List[Dict]:
        ...
        try:
            from sqlalchemy import text
            query = text("""
            SELECT pi.item_id, pi.purchase_id, pi.bar_code, pi.item_name_ocr,
                   pi.match_confidence, pi.quantity, pi.unit_price, pi.total_price,
                   pi.estimated_servings, pi.total_calories, pi.total_proteins,
                   pi.total_carbs, pi.total_fat, pr.receipt_date, pr.store_name
    ...
    def get_user_purchase_matrix(self, user_id: int) -> Dict:
        ...
        try:
            from sqlalchemy import text
            query = text("""
            SELECT pi.bar_code, COUNT(*) as purchase_count, SUM(pi.quantity) as total_quantity
            FROM purchase_item pi
            JOIN purchase_record pr ON pi.purchase_id = pr.purchase_id
            WHERE pr.user_id = :user_id
    ...
    def check_product_allergens(self, barcode: str, user_allergens: List[int]) -> Dict:
        ...
        try:
            from sqlalchemy import text
            if not user_allergens:
                return {"has_allergens": False, "detected_allergens": []}
            
            query = text("""
            SELECT pa.product_allergen_id, pa.bar_code, pa.allergen_id,
                   pa.presence_type, pa.confidence_score, a.name as allergen_name
            FROM product_allergen pa
            JOIN allergen a ON pa.allergen_id = a.allergen_id
            WHERE pa.bar_code = :barcode AND pa.allergen_id IN :allergen_ids
            """)
            
            result = self.connection.execute(query, {
                "barcode": barcode, 
                "allergen_ids": tuple(user_allergens)
            })
            rows = result.fetchall()
            
            detected_allergens = []
            for row in rows:
                detected_allergens.append(dict(row._mapping))
            ...
            logger.error(f"MySQL检查商品过敏原失败 {barcode}: {e}")
            return {"has_allergens": False, "detected_allergens": []}
    ...
    def get_allergen_by_name(self, allergen_name: str) -> Optional[Dict]:
        ...
        try:
            from sqlalchemy import text
            query = text("""
            SELECT allergen_id, name, category, is_common, description, created_time
            FROM allergen 
    ...
    def log_recommendation(self, log_data: Dict) -> bool:
        ...
        try:
            ...
            INSERT INTO recommendation_log (
                user_id, request_bar_code, request_type, recommended_products,
                algorithm_version, llm_prompt, llm_response, llm_analysis,
                processing_time_ms, total_candidates, filtered_candidates, created_at
            ) VALUES (
                :user_id, :request_bar_code, :request_type, :recommended_products,
                :algorithm_version, :llm_prompt, :llm_response, :llm_analysis,
                :processing_time_ms, :total_candidates, :filtered_candidates, NOW()
            )
            """)
            
            self.connection.execute(query, log_data)
            self.connection.commit()
            return True
    ...
    def get_recommendation_stats(self, user_id: int) -> Dict:
        ...
        try:
            ...
            if row:
                return dict(row._mapping)
            return {"total_recommendations": 0, "avg_processing_time": 0, "last_recommendation_time": None}
