...
SELECT 
    'Long barcodes (>30 chars)' AS issue_type,
    COUNT(*) AS count
FROM TEMP_PRODUCT_IMPORT 
WHERE LENGTH(barcode_raw) > 30;
...
SELECT 
    -- Clean barcode: preserve complete original value
    TRIM(barcode_raw) AS barcode,
    
    -- Clean name: preserve complete original value
...
WHERE barcode_raw IS NOT NULL 
  ...
  AND name_raw != '' 
  AND name_raw != 'NULL'
  AND LENGTH(barcode_raw) <= 30;  -- Skip barcodes that are too long
...
FROM PRODUCT 
WHERE LENGTH(barcode) > 20
ORDER BY LENGTH(barcode) DESC;
...
SELECT 'Long barcode test:' AS test_type, barcode, name, brand 
FROM PRODUCT 
WHERE LENGTH(barcode) = 24
LIMIT 1;

-- Test standard barcode query
SELECT 'Standard barcode test:' AS test_type, barcode, name, brand 
FROM PRODUCT 
...
SELECT name, brand, energy_kcal_100g, category
FROM PRODUCT 
WHERE category = 'Food' 
...
SELECT 
    '',
    'Unique Barcodes',
    CAST(COUNT(DISTINCT barcode) AS CHAR),
...
SELECT 
    ...
    CAST(MAX(LENGTH(barcode)) AS CHAR),
