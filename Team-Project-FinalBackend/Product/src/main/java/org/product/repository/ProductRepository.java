...
public interface ProductRepository extends CrudRepository<Product, String> {
     ...
     */
    @Query("SELECT p FROM Product p WHERE LOWER(p.productName) IN :names")
    List<Product> findByProductNameInIgnoreCase(@Param("names") List<String> names);

    /**
     * Count sustainable products by a list of names
     */
    @Query("SELECT COUNT(p) FROM Product p WHERE LOWER(p.productName) IN :names AND p.isSustainable = true")
    Long countSustainableProductsByNames(@Param("names") List<String> names);
}
