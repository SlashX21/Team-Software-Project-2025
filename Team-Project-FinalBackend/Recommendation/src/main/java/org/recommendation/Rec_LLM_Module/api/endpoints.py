...
async def analyze_receipt_recommendations(
    ...
    try:
        ...
        # 转换为API响应格式
        api_data = {
            "recommendationId": analysis_result.get("recommendation_id", str(uuid.uuid4())),
            "scanType": "receipt",
            "userProfileSummary": {
                "userId": request.user_id,
                "nutritionGoal": analysis_result.get("user_profile_summary", {}).get("nutrition_goal", "maintain")
            },
            # Map overall recommendations to itemAnalyses for frontend compatibility
            "itemAnalyses": [
                {
                    "product": _convert_product_to_api_format(rec.get("product", {})),
                    "reasoning": rec.get("reasoning", ""),
                    "score": rec.get("score", 0)
                }
                for rec in analysis_result.get("item_recommendations", [])
            ],
            "overallNutritionAnalysis": {
                "totalCalories": analysis_result.get("overall_nutrition_analysis", {}).get("total_calories", 0),
                "totalProtein": analysis_result.get("overall_nutrition_analysis", {}).get("total_protein", 0),
                "totalFat": analysis_result.get("overall_nutrition_analysis", {}).get("total_fat", 0),
                "goalMatchPercentage": analysis_result.get("overall_nutrition_analysis", {}).get("overall_goal_alignment", 0)
            },
            "llmInsights": {
                "summary": analysis_result.get("llm_insights", {}).get("summary", ""),
                "keyFindings": analysis_result.get("llm_insights", {}).get("key_insights", []),
                "improvementSuggestions": analysis_result.get("llm_insights", {}).get("improvement_suggestions", [])
            },
            "processingMetadata": {
                "algorithmVersion": analysis_result.get("processing_metadata", {}).get("algorithm_version", "v1.0"),
                "processingTimeMs": analysis_result.get("processing_metadata", {}).get("processing_time_ms", 0),
                "llmTokensUsed": analysis_result.get("processing_metadata", {}).get("llm_tokens_used", 0),
                "confidenceScore": analysis_result.get("processing_metadata", {}).get("confidence_score", 0.8)
            }
        }
